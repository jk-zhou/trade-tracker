...
export default function CalendarPnL({ data, dict }: { data: DailyPerformance[], dict: any }) {
  ...
  return (
    <div className="bg-card border border-border p-4 md:p-6 rounded-xl shadow-sm mb-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-xl font-bold">{dict.calendarView || 'Calendar View'}</h2>
      ...
      <div className="grid grid-cols-7 gap-1 md:gap-2">
        {days.map((day, i) => {
          ...
          const pnl = dayData?.dailyRealizedPnL || 0;
          ...
          if (pnl > 0) {
            bgColor = 'bg-success/20 border-success/30 border';
            textColor = 'text-success font-semibold';
          } else if (pnl < 0) {
            bgColor = 'bg-destructive/20 border-destructive/30 border';
          ...
          if (isToday) {
            bgColor += ' ring-2 ring-primary ring-offset-1 ring-offset-background';
          }

          return (
            <div 
              key={dateStr} 
              className={`flex flex-col h-16 md:h-24 p-1 md:p-2 rounded-lg transition-colors ${bgColor}`}
            >
              <span className={`text-xs md:text-sm self-end mb-auto ${isCurrentMonth ? 'opacity-80' : 'opacity-40'}`}>
                {format(day, 'd')}
              </span>
              {pnl !== 0 && (
                <span className={`text-[10px] md:text-sm text-center truncate ${textColor}`}>
